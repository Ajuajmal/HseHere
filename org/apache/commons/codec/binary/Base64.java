...
public class Base64 implements BinaryEncoder, BinaryDecoder {
    ...
    public Base64(int lineLength, byte[] lineSeparator, boolean urlSafe) {
        ...
        if (containsBase64Byte(lineSeparator)) {
            throw new IllegalArgumentException(new StringBuffer().append("lineSeperator must not contain base64 characters: [").append(StringUtils.newStringUtf8(lineSeparator)).append("]").toString());
        }
    ...
    void encode(byte[] in, int inPos, int inAvail) {
        if (!this.eof) {
            ...
            if (inAvail < 0) {
                ...
                switch (this.modulus) {
                    case 1:
                        ...
                        i = this.pos;
                        this.pos = i + 1;
                        bArr[i] = this.encodeTable[(this.f70x >> 2) & 63];
                        bArr = this.buffer;
                        i = this.pos;
                        this.pos = i + 1;
                        bArr[i] = this.encodeTable[(this.f70x << 4) & 63];
                    ...
                    case 2:
                        ...
                        i = this.pos;
                        this.pos = i + 1;
                        bArr[i] = this.encodeTable[(this.f70x >> 10) & 63];
                        bArr = this.buffer;
                        i = this.pos;
                        this.pos = i + 1;
                        bArr[i] = this.encodeTable[(this.f70x >> 4) & 63];
            ...
            while (i2 < inAvail) {
                ...
                this.f70x = (this.f70x << 8) + b;
                if (this.modulus == 0) {
                    bArr = this.buffer;
                    i = this.pos;
                    this.pos = i + 1;
                    bArr[i] = this.encodeTable[(this.f70x >> 18) & 63];
                    bArr = this.buffer;
                    i = this.pos;
                    this.pos = i + 1;
                    bArr[i] = this.encodeTable[(this.f70x >> 12) & 63];
                    bArr = this.buffer;
                    i = this.pos;
                    this.pos = i + 1;
                    bArr[i] = this.encodeTable[(this.f70x >> 6) & 63];
                    bArr = this.buffer;
                    i = this.pos;
                    this.pos = i + 1;
                    bArr[i] = this.encodeTable[this.f70x & 63];
            ...
            while (i2 < inAvail) {
                ...
                if (b >= (byte) 0 && b < DECODE_TABLE.length) {
                    ...
                    if (result >= 0) {
                        ...
                        this.modulus = i3;
                        this.modulus = i3 % 4;
                        this.f70x = (this.f70x << 6) + result;
                        if (this.modulus == 0) {
                            bArr = this.buffer;
                            i = this.pos;
                            this.pos = i + 1;
                            bArr[i] = (byte) ((this.f70x >> 16) & 255);
                            bArr = this.buffer;
                            i = this.pos;
                            this.pos = i + 1;
                            bArr[i] = (byte) ((this.f70x >> 8) & 255);
                            bArr = this.buffer;
                            i = this.pos;
                            this.pos = i + 1;
                            bArr[i] = (byte) (this.f70x & 255);
                        }
                    }
                }
                i2++;
    ...
    public static byte[] encodeBase64(byte[] binaryData, boolean isChunked, boolean urlSafe) {
        return encodeBase64(binaryData, isChunked, urlSafe, Strategy.TTL_SECONDS_INFINITE);
    }
    ...
    public static byte[] encodeBase64(byte[] binaryData, boolean isChunked, boolean urlSafe, int maxResultSize) {
        ...
        if (len > ((long) maxResultSize)) {
            throw new IllegalArgumentException(new StringBuffer().append("Input array too big, the output array would be bigger (").append(len).append(") than the specified maxium size of ").append(maxResultSize).toString());
        }
