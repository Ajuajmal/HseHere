...
public class zzjv extends WebChromeClient {
    ...
    public final boolean onConsoleMessage(ConsoleMessage consoleMessage) {
        ...
        switch (C05137.zzOk[consoleMessage.messageLevel().ordinal()]) {
            case 1:
                zzb.m252e(str);
                break;
            case 2:
    ...
    public final void onExceededDatabaseQuota(String url, String databaseIdentifier, long currentQuota, long estimatedSize, long totalUsedQuota, QuotaUpdater quotaUpdater) {
        ...
        if (currentQuota == 0) {
            if (estimatedSize > j || estimatedSize > C0204c.fB) {
                estimatedSize = 0;
            }
        } else if (estimatedSize == 0) {
            estimatedSize = Math.min(Math.min(131072, j) + currentQuota, C0204c.fB);
        } else {
            if (estimatedSize <= Math.min(C0204c.fB - currentQuota, j)) {
